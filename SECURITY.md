...
- Two checksummed boot-state records select an A/B system slot. A damaged active slot is rejected before execution and the independently signed fallback is tried. If both boot-state records are damaged, the loader scans signed A/B slots and selects the highest verified release allowed by the persisted release floor.
